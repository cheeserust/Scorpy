...
정상 이동 중 limit switch는 축별 nonfatal 제약이다. limit 방향으로 움직이던
해당 축만 current position에서 멈추고 다른 축은 계속 이동한다. global error가
발생하지 않으므로 Clear Error 없이 반대 방향 명령을 보내 limit에서 빠져나올
수 있다. Status Byte4 raw limit bit를 UI에 표시하되, blocked 축의 원래 target
미도달을 전체 goal 실패로 강제할 필요는 없다.
