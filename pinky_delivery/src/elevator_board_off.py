...
ROTATE_ANGLE_DEG  = 90.0         # 후진 하차 --> 회전 --> 주행
ROTATE_ANGULAR_SPEED = 0.5        # rad/s
ROTATE_DURATION   = math.radians(ROTATE_ANGLE_DEG) / ROTATE_ANGULAR_SPEED
# ────────────────────────────────────────────────────────────
...
DONE       = "DONE"
ROTATING   = "ROTATING"     # 회전 중 (후진 하차 후)
    ...
    def control_loop(self):
        if self.state == WAIT_BOARD:
            ...
            if elapsed < ROTATE_DURATION:
                self.drive(linear=0.0, angular=ROTATE_ANGULAR_SPEED)
            else:
                self.stop()
                self.state = DONE
                self.get_logger().info("회전 완료 → DONE")
