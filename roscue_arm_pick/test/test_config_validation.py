...
def test_project_arm_stream_uses_seven_ms_frames_and_forty_ms_points():
    params = load_yaml(
        str(WORKSPACE_SRC / 'arm_can_bridge/config/arm_can_bridge.yaml')
    )['arm_can_bridge']['ros__parameters']

    assert params['arm_inter_frame_delay_ms'] == 7.0
    assert params['arm_trajectory_point_duration_ticks'] == 8
    assert params['arm_trajectory_min_duration_ticks'] == 8
