...
> **주의:** Board1/Board2 이동 명령 부분은 폐기됐다. 현재 이동 wire contract는
> `CAN_GOAL_V3_PROTOCOL.md`가 유일한 기준이다. 이 문서에서는 feedback 및 Board3
> 참고 정보만 사용하고, 구 queue/Byte5~7 command 형식을 구현하지 않는다.
...
E-stop frame은 DLC 8이며 payload는 다음과 같습니다.
...
- 모든 STEP 출력 정지
- trajectory goal, staging, homing 및 pending motion clear
- 이전 goal의 software ACK queue clear
- E-stop 직전 motor enable 상태 유지
- 상태를 ESTOP으로 전환
- 해당 보드 status 즉시 송신, Board1은 `0x201`, Board2는 `0x202`, Board3는 `0x203`

Enabled 상태에서 E-stop이 들어오면 모터 EN은 계속 활성 상태이므로 holding
torque가 유지됩니다. Disabled 상태에서 E-stop이 들어오면 모터를 자동으로
Enable하지 않습니다. 이 기능은 안전 인증된 STO가 아니라 팀 프로젝트용
software powered hold입니다.

E-stop은 `0x010 Enable=1` 또는 `0x030 Clear Error` 중 하나를 수신하면
해제됩니다. 해제 후 취소된 goal이나 homing은 자동 재시작하지 않습니다.
엔코더가 없는 stepper의 `current_step`은 명령 기반 추정 위치이므로 즉시 정지
중 탈조가 발생하면 실제 위치와 차이가 날 수 있습니다.
...
Disable 수신 시 STM32는 goal/homing clear, step 정지, motor disable을
수행합니다. Disable은 관리자 또는 사용자의 명시적 명령에서만 보내며 E-stop,
timeout, retry 실패, BUSY, INVALID 또는 통신 종료 시 자동 전송하지 않습니다.
...
정상 이동 중 축별 home 방향 리미트가 감지되면 해당 축만 즉시 정지하고 그
축의 target을 current position으로 변경합니다. 전역 error를 설정하거나 다른
축을 정지하지 않습니다. 반대 방향 이동은 스위치가 눌린 상태에서도 허용되므로
Clear Error 없이 리미트에서 빠져나올 수 있습니다. 서버는 이 경우 원래 target
미도달 축이 있어도 전체 goal을 완료로 관찰할 수 있습니다.
...
Clear Error는 모션/goal을 정리하고 error code와 ESTOP flag를 해제합니다. 모터
enable 상태는 변경하지 않습니다. 따라서 enabled 상태에서는 `STATE_IDLE`,
disabled 상태에서는 `STATE_DISABLED`로 전환하며 이전 작업은 자동 재시작하지
않습니다.
