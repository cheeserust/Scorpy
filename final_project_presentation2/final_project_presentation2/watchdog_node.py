...
def _source_command_age_sec(
    ...
    max_age_sec: Any,
    *,
    allow_unsynchronized_clock: bool = False,
) -> Optional[float]:
    ...
    try:
        ...
        return 0.0 if allow_unsynchronized_clock else None
    if source <= 0.0:
        return 0.0 if allow_unsynchronized_clock else None
    age = now - source
...
class FinalProjectPresentationWatchdog(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter('allow_unsynchronized_clock', False)
        self._allow_unsynchronized_clock = bool(
            self.get_parameter('allow_unsynchronized_clock').value
        )
        ...
        if self._allow_unsynchronized_clock:
            self.get_logger().warning(
                'DEMO RELAXATION ENABLED: commands with an unusable source '
                'timestamp will be aged from local receipt time; synchronize '
                'PC and Pinky clocks and disable this option after the demo'
            )
    ...
    def _raw_callback(self, message: TwistStamped) -> None:
        ...
        source_age = _source_command_age_sec(
            ...
            now_ros_sec,
            max_age,
            allow_unsynchronized_clock=self._allow_unsynchronized_clock,
        )
