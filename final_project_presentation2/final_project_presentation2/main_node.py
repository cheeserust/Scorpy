...
class PresentationNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter('allow_degraded_drive', False)
        self._allow_degraded_drive = bool(
            self.get_parameter('allow_degraded_drive').value
        )
        ...
        self._degraded_graph_warning_active = False
        ...
        if self._allow_degraded_drive:
            self._record_event(
                'warning',
                'DEMO DEGRADED DRIVE ENABLED: odometry and central velocity '
                'graph readiness do not block straight routes or workflows; '
                'exact turns still require odometry',
            )
    ...
    def _publish_command(self) -> None:
        ...
        with self._lock:
            ...
            if graph is not None and not graph['ready']:
                if self._allow_degraded_drive:
                    if not self._degraded_graph_warning_active:
                        self._degraded_graph_warning_active = True
                        graph_warning = (
                            'Demo degraded drive is ignoring the central '
                            'velocity publisher graph readiness error'
                        )
                else:
                    ...
                    if self._safety_abort_reason is None:
                        graph_error = (
                            ...
                            'motion: expected the presentation node as the '
                            'sole raw publisher and the watchdog as the sole '
                            '/cmd_vel publisher'
                        )
        ...
        if graph_warning is not None:
            self._record_event('warning', graph_warning)
    ...
    def _readiness(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            drive_required = (
                ...
                if getattr(self, '_allow_degraded_drive', False)
                else strict_drive_required
            )
            turn_required = (
                'config',
                'odom',
            ...
            readiness['degraded_drive_enabled'] = bool(
                getattr(self, '_allow_degraded_drive', False)
            )
