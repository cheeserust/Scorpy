...
시연 직전 시간 동기화를 복구할 수 없는 경우에만 Pinky watchdog을 다음과
같이 실행할 수 있습니다.

```bash
ros2 launch final_project_presentation2 \
  final_project_presentation2_watchdog.launch.py \
  allow_unsynchronized_clock:=true
```

이 옵션은 사용할 수 없는 source timestamp를 Pinky의 로컬 수신 시각으로
대체합니다. 0.25초 로컬 command timeout, 속도 제한, raw 및 `/cmd_vel`
publisher 독점 검사는 계속 적용되지만 지연되어 도착한 source command를
timestamp로 거부하는 보호는 약해집니다. 시연 후에는 양쪽 시계를 동기화하고
기본값 `false`로 되돌리십시오.

중앙 PC의 상태 화면에서 odometry 또는 velocity publisher graph가 오류여도
직선 경로와 워크플로우를 시작해야 하는 시연에서는 중앙 노드를 다음과 같이
실행할 수 있습니다.

```bash
ros2 launch final_project_presentation2 final_project_presentation2.launch.py \
  allow_degraded_drive:=true
```

상태 화면에는 원래 오류가 계속 표시되지만 직선 경로의 readiness와 실행 중
중앙 velocity graph 강제 종료에서는 제외됩니다. odometry가 실제로 필요한
정확 회전은 계속 차단됩니다. 이 옵션은 다른 `/cmd_vel` publisher와 충돌할
가능성을 허용하므로 물리 E-stop 담당자가 있는 시연에서만 사용하십시오.
