...
def test_demo_clock_relaxation_falls_back_to_local_receipt_age():
    options = {'allow_unsynchronized_clock': True}

    assert _source_command_age_sec(
        99.8, 100.0, 0.25, **options
    ) == pytest.approx(0.2)
    assert _source_command_age_sec(99.0, 100.0, 0.25, **options) == 0.0
    assert _source_command_age_sec(101.0, 100.0, 0.25, **options) == 0.0
    assert _source_command_age_sec(0.0, 100.0, 0.25, **options) == 0.0
