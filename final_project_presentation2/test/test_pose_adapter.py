...
def test_degraded_drive_does_not_abort_on_central_graph_error():
    document = _template()
    messages = []
    events = []
    node = object.__new__(PresentationNode)
    ...
    node._return_confirm_event = threading.Event()
    node._safety_abort_reason = None
    node._allow_degraded_drive = True
    node._degraded_graph_warning_active = False
    node._velocity_graph_status = lambda: {'ready': False}
    node._raw_cmd_publisher = SimpleNamespace(publish=messages.append)
    node._pose_executor = SimpleNamespace(cancel_active=lambda **_kwargs: True)
    node._record_event = lambda level, message: events.append((level, message))
    stamp = SimpleNamespace(sec=100, nanosec=0)
    ...
    assert len(messages) == 1
    assert messages[0].twist.linear.x == pytest.approx(0.08)
    assert messages[0].twist.angular.z == pytest.approx(0.20)
    assert node._safety_abort_reason is None
    assert not node._stop_event.is_set()
    assert events == [(
        'warning',
        'Demo degraded drive is ignoring the central velocity publisher '
        'graph readiness error',
    )]
