...
class VicPinkyNavAdapter(Node):
    ...
    def _execute_callback(self, goal_handle) -> RunTask.Result:
        ...
        if goal_handle.is_cancel_requested:
            if send_response is not None and send_response.accepted:
                self._cancel_nav_goal(send_response)
            else:
                self._cancel_late_nav_goal(send_future)
    ...
    def _cancel_late_nav_goal(self, send_future) -> None:
        """Cancel a Nav2 goal that is accepted after this task has stopped."""
        def cancel_when_available(completed_future):
            try:
                nav_goal_handle = completed_future.result()
